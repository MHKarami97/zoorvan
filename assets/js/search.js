(function () {
  ...
  fetch(window.SEARCH_JSON_URL)
    ...
    .then(function (json) {
      data = json;
      status.textContent =
        data.length + " تجربه ثبت‌شده — عبارت خود را تایپ کنید";
    })
    .catch(function () {
  ...
  function render(items) {
    ...
    if (items.length === 0) {
      results.innerHTML =
        '<p style="color:var(--color-grey-500); grid-column:1/-1;">نتیجه‌ای پیدا نشد.</p>';
      return;
    }
  ...
  function search(term) {
    ...
    var filtered = data.filter(function (exp) {
      var haystack = [
        ...
        exp.excerpt,
        (exp.tags || []).join(" "),
        window.CATEGORY_MAP[exp.category] || exp.category,
      ]
        .join(" ")
        .toLowerCase();
